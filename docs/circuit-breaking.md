...
| Intermittent network weather causes the Petshop Service to become unacceptably latent for all Callers. | Incoming calls that are timing out will eventually cause the `SOURCE->Petshop::latentMethod` Circuits to become unhealthy. Circuit alerts will prompt engineers that something is wrong, and the Petshop Service will deny traffic to all Callers that experienced latent calls until the Circuits automatically become Healthy again. Diagnosis should occur afterwards, and the root cause of the network weather fixed. |
